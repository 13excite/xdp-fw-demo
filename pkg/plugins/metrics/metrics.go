...
const (
	...
	// default length of vector values in seconds
...
// MetricsVector stores the last "length" values of some keys.
...
func (j *MetricsWorker) Push(metricID int, tags []string, value float64) {
	...
	if _, ok := j.values[key]; !ok {
		...
		j.values[key][0] = append(j.values[key][0], float64(0))
		interval = 0
	}
...
// GetClientMetrics fetches metrics from the running daemon via
// its http api.
